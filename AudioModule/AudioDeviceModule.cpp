...
int32_t
AudioDeviceModule::PlayoutDeviceName(uint16_t index, char name[kAdmMaxDeviceNameSize], char guid[kAdmMaxGuidSize]) {
    return 0;
}

int32_t
AudioDeviceModule::RecordingDeviceName(uint16_t index, char name[kAdmMaxDeviceNameSize], char guid[kAdmMaxGuidSize]) {
    return 0;
}
...
int32_t AudioDeviceModule::InitRecording(uint8_t channel, uint32_t SampleRate, uint8_t BytesPerSample) {
    _GetDefaultMicDevice(&m_capture_device);
    HRESULT hr = E_FAIL;
